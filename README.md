...
For a given value of c, the corresponding Julia set is the set of initial values $z_0$ for which the sequence is bounded. This distinction leads us to define the following sets: $Q_c^{(-k)}$ = [ $z_0$ tq | $z_k$ | <= R(c)]

The set of prisoner points (i.e., the z0 that give a bounded sequence) corresponds to the limit where k tends to infinity.

In the following figure, we have illustrated (by a gray level "k+1") the sets $Q_c^{(-k)}$ associated to the constant c=i:
